...
module Nvd3
  module Helper
    ...
    def areachart(data_source, options = {})
      nvd3_chart "AreaChart", data_source, options
    end
    ...
    def nvd3_chart(klass, data_source, options)
      ...
      element_id = options.delete(:id) || "d3chart-#{@chartkick_chart_id += 1}"
      height = options.delete(:height) || "300px"
      # content_for: nil must override default
      content_for = options.key?(:content_for) ? options.delete(:content_for) : Nvd3.content_for

      html = (options.delete(:html) || %(<div id="%{id}" style="height: %{height}; >Loading nvd3...</div>)) % {id: ERB::Util.html_escape(element_id), height: ERB::Util.html_escape(height)}

      # data_source = data_source.respond_to?(:chart_json) ? data_source.chart_json : data_source.to_json
      data_source = [{
                         key: 'testname',
                         values: data_source.to_a
                     }]

      js = <<JS
<script type="text/javascript">
  nvd3.create({
    name: #{element_id.to_json},
    data: #{data_source},
